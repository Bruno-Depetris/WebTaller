...
async function loadDetalleReparacion() {
    ...
    try {
        ...
        // Obtener catálogos para mostrar nombres en vez de IDs
        const [reparaciones, servicios, repuestos] = await Promise.all([
            apiRequest('Reparacione'),
            apiRequest('Servicio'),
            apiRequest('Repuesto')
        ]);
        // Mapear IDs a valores legibles
        const reparacionMap = {};
        reparaciones.forEach(r => reparacionMap[r.idReparacion] = `#${r.idReparacion} - ${r.estado || ''}`);
        const servicioMap = {};
        servicios.forEach(s => servicioMap[s.idServicio] = s.nombre);
        const repuestoMap = {};
        repuestos.forEach(r => repuestoMap[r.idRepuesto] = r.nombre);

        // Construir datos con valores legibles
        const formattedData = data.map(item => ({
            idDetalle: item.idDetalle,
            reparacion: reparacionMap[item.idReparacion] || item.idReparacion,
            servicio: item.idServicio ? (servicioMap[item.idServicio] || item.idServicio) : '',
            repuesto: item.idRepuesto ? (repuestoMap[item.idRepuesto] || item.idRepuesto) : '',
